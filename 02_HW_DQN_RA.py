...
# ----------------------------------------截图下限-----------------------------------------------------------------------
        s = s_
        print('*  DQN_RA total_step_times:', config.total_step_times)
################################  开始训练  ###################################################
        if dqn.position >= MEMORY_CAPACITY:  # 池子满了，开始训练
            ...
            if (config.trained_times == total_train_times + 1 ):  # 全部训练完成，显示结果。跳出循环
                ...
                #     ws.cell(row=i + 1, column=1).value = float(avg_loss_episode_array[i])  # 在第1行第1列写入LOSS
                #     wb.save('RADQN-LOSS.xlsx')  # 保存excel表

                # for j in range(config.nspeed):
                #     ws.cell(row=j + 1, column=1).value = float(config.avg_speed_episode_array[j])  #
                #     wb.save('02-DQN-RA-speed.xlsx')  # 保存excel表

                for h in range(config.nreward):
                    ws.cell(row=h + 1, column=1).value = float(config.sum_reward_episode_array[h])  #
                    wb.save('02-DQN-RA-reward.xlsx')  # 保存excel表

                for i in range(config.nRA):
                    ws.cell(row=i + 1, column=1).value = float(config.sum_RA_episode_array[i])  #
                    wb.save('02-DQN-RA-RA.xlsx')  # 保存excel表

                for j in range(config.ntm):
                    ws.cell(row=j + 1, column=1).value = float(config.tm_ep_array[j])  #
                    wb.save('02-DQN-RA-tmep.xlsx')  #
