...
# ----------------------------------------截图下限-----------------------------------------------------------------------
        s = s_
        print('*  DQN-SPD  total_step_times:', config.total_step_times)
################################  开始训练  ###################################################
        if dqn.position >= MEMORY_CAPACITY:  # 池子满了，开始训练
            ...
            if (config.trained_times == total_train_times + 1 ):  # 全部训练完成，显示结果。跳出循环
                ...
                # plt.xlabel('DQN AR trained episodes')
                # plt.ylabel('avg_loss')
                # plt.subplot(3, 1, 2)
                ...
                #     wb.save('RRDQN-LOSS.xlsx')  # 保存excel表

                for k in range(config.nspeed):
                    ws.cell(row=k + 1, column=1).value = float(config.avg_speed_episode_array[k])  #S
                    wb.save('02-DQN-SPD-speed.xlsx')  #

                for l in range(config.nreward):
                    ws.cell(row=l + 1, column=1).value = float(config.sum_reward_episode_array[l])  #
                    wb.save('02-DQN-SPD-reward.xlsx')  #

                # for l in range(config.nRA):
                #     ws.cell(row=l + 1, column=1).value = float(config.sum_RA_episode_array[l])  #
                #     wb.save('02-DQN-SPD-RA.xlsx')  #

                for m in range(config.ntm):
                    ws.cell(row=m + 1, column=1).value = float(config.tm_ep_array[m])  #
                    wb.save('02-DQN-SPD-tmep.xlsx')  #
