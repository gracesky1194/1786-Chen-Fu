...
# ----------------------------------------截图下限-----------------------------------------------------------------------
        s = s_
        print('*  DDQN-SPD total_step_times:', config.total_step_times)
################################  开始训练  ###################################################
        if ddqn.position >= MEMORY_CAPACITY:  # 池子满了，开始训练
            ...
            if (config.trained_times == total_train_times + 1 ):  # 全部训练完成，显示结果。跳出循环
                ...
                # plt.xlabel('DDQN AR trained episodes')
                # plt.ylabel('avg_loss')
                # plt.subplot(3, 1, 2)
                ...
                for t in range(config.nspeed):
                    ws.cell(row=t + 1, column=1).value = float(config.avg_speed_episode_array[t])  #S
                    wb.save('03-DDQN-SPD-speed.xlsx')  # 保存excel表

                for u in range(config.nreward):
                    ws.cell(row=u + 1, column=1).value = float(config.sum_reward_episode_array[u])  #
                    wb.save('03-DDQN-SPD-reward.xlsx')  # 保存excel表

                # for l in range(config.nRA):
                #     ws.cell(row=l + 1, column=1).value = float(config.sum_RA_episode_array[l])  #
                #     wb.save('03-DDQN-SPD-RA.xlsx')  # 保存excel表

                for v in range(config.ntm):
                    ws.cell(row=v + 1, column=1).value = float(config.tm_ep_array[v])  #
                    wb.save('03-DDQN-SPD-tmep.xlsx')  #
