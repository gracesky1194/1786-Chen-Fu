...
while True:  # 原始程序
    ...
    while not done:
        ...
        if (config.start_train_TR == 0):   # 原始 训练前
            ...
            config.lane_old = r[1]
            config.new_reward = r[0]  # 只有碰撞奖励
            # print('config.lane_old:', config.lane_old)

            if not done:  # 不碰撞，正确动作
                dqn.push_memory(s, a, config.new_reward, s_)
                config.memory_cnt += 1  #
            else:  # 碰撞，错误动作
                config.memory_cnt += 1
                dqn.push_memory(s, a, config.new_reward, s_)

                # config.E_tm_ep = time.time()  # 训练签的时间没有意义
        ...
        #========================# 训练后 #===========================#
            ...
            if (config.lane_old == 0 and a == 0) or (config.lane_old == 2 and a == 2):  # 不合理动作
                ...
                config.correct_act_cnt_in_train += 1   # RA总计数器
                config.sum_RA_episode += 1  # 轮次RA计数器
            ...
            if not done:  # 无碰撞，正确动作
                ...
                # config.E_tm_ep = time.time()     # 计算每一轮时间，不管撞和成功
                # config.tm_ep = config.E_tm_ep - config.S_tm_ep
                # config.tm_ep_array.append(config.tm_ep)
                # config.ntm += 1

                if np.all(s_[1] == [0, 0, 0, 0]) & np.all(s_[2] == [0, 0, 0, 0]):  # env.commom.abstract 中定义 obs各旁车的坐标，都为零即不出现，说明避障成功。
                    config.success_done += 1
...
# ----------------------------------------截图下限-----------------------------------------------------------------------
        s = s_
        print('*  DQN total_step_times:', config.total_step_times)
################################  开始训练  ###################################################
        if dqn.position >= MEMORY_CAPACITY:  # 池子满了，开始训练
            ...
            if (config.trained_times == total_train_times + 1 ):  # 全部训练完成，显示结果。跳出循环 40W
                end_time = time.time()
                ...
                print('      训练次数:', config.trained_times - 1)  # 池子满之后的训练次数
                ...
                #     wb.save('DQN-loss.xlsx')  # 保存excel表

                for a in range(config.nspeed):
                    ws.cell(row=a + 1, column=1).value = float(config.avg_speed_episode_array[a])  #
                    wb.save('02-DQN-0speed.xlsx')  # 保存excel表

                # for b in range(config.nRA):
                #     ws.cell(row=b + 1, column=1).value = float(config.sum_RA_episode_array[b])  #
                #     wb.save('02-DQN-0RA.xlsx')  # 保存excel表

                for c in range(config.ntm):
                    ws.cell(row=c + 1, column=1).value = float(config.tm_ep_array[c])  #
                    wb.save('02-DQN-0tmep.xlsx')  #
                ...
                #     ws.cell(row=k + 1, column=2).value = float(config.sum_reward_episode_array[k])  #
                #     wb.save('02-DQN-reward.xlsx')  # 保存excel表
...
'''
################  Just an example, no contribution to the used code  ###################
# for _ in range(10):
...
# plt.show()
# ACTIONS_ALL = { 0: 'LANE_LEFT', 1: 'IDLE', 2: 'LANE_RIGHT',
#                 3: 'FASTER',               4: 'SLOWER'   }
################################ DQN  ##################################################
...
# print('total_time:', episode_time)
#################################################################
# plt.figure()
# plt.title('Highway_env_DQN loss')
#
# plt.subplot(3,1,1)
# plt.plot(loss_value)
...
#
# plt.subplot(3,1,2)
# plt.plot(loss_value_50_avg)
# plt.xlabel('trained times')
# plt.ylabel('loss_value_avg')
#
