...
env.config["collision_reward"] = config.collision_reward  # -0.1
env.config["high_speed_reward"] = config.high_speed_reward  #
...
while True:  # 原始程序
    ...
    while not done:
        ...
        if ( config.start_train_TR == 0):  # AR 训练前
            ...
            config.new_reward = r[0] + env.config["correct_act_reward"] * config.CORRECT_ACT_TR
            config.reward = utils.lmap(config.new_reward,
                                [env.config["collision_reward"] ,  env.config["correct_act_reward"]],
                                [0, 1])
            # config.new_reward = r[0] + env.config["wrong_act_reward"] * config.WRO_ACT_TR + env.config["correct_act_reward"] * config.CORRECT_ACT_TR
            # config.reward = utils.lmap(config.new_reward,
            #                     [env.config["collision_reward"] + env.config["wrong_act_reward"],  env.config["correct_act_reward"]],
            #                     [0, 1])
...
# ----------------------------------------截图下限-----------------------------------------------------------------------
        s = s_
        print('*  DuDQN-RA  total_step_times:',  config.total_step_times)
################################  开始训练  ###################################################
        if dddqn.position >= MEMORY_CAPACITY:  # 池子满了，开始训练
            ...
            if (config.trained_times == total_train_times + 1):  # 全部训练完成，显示结果。跳出循环
                ...
                #     wb.save('RADuDQN-LOSS.xlsx')  # 保存excel表

                # for j in range(config.nspeed):
                #     ws.cell(row=j + 1, column=1).value = float(config.avg_speed_episode_array[j])  #
                #     wb.save('04-DuDQN-RA-speed.xlsx')  # 保存excel表

                for dd in range(config.nreward):
                    ws.cell(row=dd + 1, column=1).value = float(config.sum_reward_episode_array[dd])  #
                    wb.save('04-DuDQN-RA-reward.xlsx')  # 保存excel表

                for ee in range(config.nRA):
                    ws.cell(row=ee + 1, column=1).value = float(config.sum_RA_episode_array[ee])  #
                    wb.save('04-DuDQN-RA-RA.xlsx')  # 保存excel表

                for ff in range(config.ntm):
                    ws.cell(row=ff + 1, column=1).value = float(config.tm_ep_array[ff])  #
                    wb.save('04-DuDQN-RA-tmep.xlsx')  #
