...
while True:  # 原始程序
    ...
    while not done:
        ...
        #========================# 训练后 #===========================#
            ...
            if (config.lane_old == 0 and a == 0) or (
                    config.lane_old == env.config["lanes_count"] - 1 and a == 2):  # 如果出现不合理动作，触发惩罚标记 WRO_ACT_TR
...
# ----------------------------------------截图下限-----------------------------------------------------------------------
        s = s_
        print('*  DuDQN_C total_step_times:', config.total_step_times)
################################  开始训练  ###################################################
        if dddqn.position >= MEMORY_CAPACITY:  # 池子满了，开始训练
            ...
            if (config.trained_times == total_train_times + 1 ):  # 全部训练完成，显示结果。跳出循环
                ...
                #     wb.save('RSDuDQN-LOSS.xlsx')  # 保存excel表

                for z in range(config.nspeed):
                    ws.cell(row=z + 1, column=1).value = float(config.avg_speed_episode_array[z])  #
                    wb.save('04-DuDQN-C-speed.xlsx')  # 保存excel表

                for aa in range(config.nreward):
                    ws.cell(row=aa + 1, column=1).value = float(config.sum_reward_episode_array[aa])  #
                    wb.save('04-DuDQN-C-reward.xlsx')  # 保存excel表

                for bb in range(config.nRA):
                    ws.cell(row=bb + 1, column=1).value = float(config.sum_RA_episode_array[bb])  #
                    wb.save('04-DuDQN-C-RA.xlsx')  # 保存excel表

                for cc in range(config.ntm):
                    ws.cell(row=cc + 1, column=1).value = float(config.tm_ep_array[cc])  #
                    wb.save('04-DuDQN-C-tmep.xlsx')  #
