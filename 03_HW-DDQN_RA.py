...
# ----------------------------------------截图下限-----------------------------------------------------------------------
        s = s_
        print('*  DDQN-RA total_step_times:', config.total_step_times)
################################  开始训练  ###################################################
        if ddqn.position >= MEMORY_CAPACITY:  # 池子满了，开始训练
            ...
            if (config.trained_times == total_train_times + 1):  # 全部训练完成，显示结果。跳出循环
                ...
                #     wb.save('RRDDQN-LOSS.xlsx')  # 保存excel表

                # for j in range(config.nspeed):
                #     ws.cell(row=j + 1, column=1).value = float(config.avg_speed_episode_array[j])  #
                #     wb.save('03-DDQN-RA-speed.xlsx')  # 保存excel表

                for r in range(config.nreward):
                    ws.cell(row=r + 1, column=1).value = float(config.sum_reward_episode_array[r])  #
                    wb.save('03-DDQN-RA-reward.xlsx')  # 保存excel表

                for r in range(config.nRA):
                    ws.cell(row=r + 1, column=1).value = float(config.sum_RA_episode_array[r])  #
                    wb.save('03-DDQN-RA-RA.xlsx')  # 保存excel表

                for s in range(config.ntm):
                    ws.cell(row=s + 1, column=1).value = float(config.tm_ep_array[s])  #
                    wb.save('03-DDQN-RA-tmep.xlsx')  #
